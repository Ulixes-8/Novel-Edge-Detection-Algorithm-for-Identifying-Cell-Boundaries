...
def split_data(data_dir,augment_both=True):
    ...
    shutil.copytree(os.path.join(img_dir, 'real'), img_aug_dir+'/real', dirs_exist_ok=True)
    ...
    for i in range(n):
        image_path = os.path.join(os.path.join(img_dir, 'real'), x_list[i])
        print(f"Loading image from: {image_path}")
        x_tmp = cv.imread(image_path)
            # Check if the image was loaded successfully
        if x_tmp is None:
            print(f"Failed to load image from: {image_path}. Please check if the file exists and is accessible.")
